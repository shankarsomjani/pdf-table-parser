        ...
        if not LLM_API_KEY:
            ...
            try:
                with st.spinner("🔄 Sending file to LLMWhisperer..."):
                    ...
                    # Process tables from result_text
                    import re
                    ...
                    for idx, table_text in enumerate(tables):
                        try:
                            ...
                            if df.shape[1] > 1:
                                dataframes.append((f"LLM_Table{idx+1}", df))
                        except Exception:
                            continue
                    ...
                    if dataframes:
                        ...
                        with pd.ExcelWriter(output, engine='openpyxl') as writer:
                            for name, df in dataframes:
                                df.to_excel(writer, sheet_name=name[:31], index=False)
                        st.download_button("📅 Download Extracted Tables", output.getvalue(), "llm_tables.xlsx")
                    else:
                        st.warning("⚠️ No tables detected in LLMWhisperer result.")
